from Train_Test_Data import train_datasets, test_datasets # to load files from Train_Test_Data.py
from Train_Test_Data import drug_list  # to load files from Train_Test_Data.py


# Imports from TUGDA itself
import torch
...
# First idea to include gnn-class into MTL of TUGDA with Encoder
# Problems: long runtime, possibly too much data in gnn (Overfitting)

class GNNEncoder(nn.Module):
    '''
    - uses Graph Attention Convolution (GATConv) layers combined with TopKPooling 
      to learn hierarchical graph representations by attending over neighbors and selecting important nodes
    - applies global max pooling and global average pooling to aggregate node features into embeddings
    - fully connected layers for final projection
    '''
