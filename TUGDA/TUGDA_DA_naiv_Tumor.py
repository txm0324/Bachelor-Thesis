...
drugs_pdx = pdx_dataset_brca.columns[1780:]
...
# Pathway-Interaction:
X_train = extension_with_multiple_task_features(X_train, y_train, task_feature_matrices=[dgi_matrix_indirect_05], weights=[1])
# Combination
...
X_test = pdx_dataset_pdac[gene_list].values
y_test = pdx_dataset_pdac[drugs_pdx].values

X_train_unl = pdx_dataset_pdac[gene_list].values
y_train_unl_dummy = np.zeros((X_train_unl.shape[0], y_train.shape[1]))  # Dummy labels
X_train_unl = extension_with_multiple_task_features(X_train_unl, y_train_unl_dummy, task_feature_matrices=[dgi_matrix_indirect_05], weights=[1])
...
df_preds.index = [f"Patient_{i}" for i in range(39)]
df_genes_pdx = pdx_dataset_pdac.iloc[:, :1780].copy()
df_preds.index = df_genes_pdx.index

# Name for specific variant
df_preds.to_csv('./preds_AUC_indirect_PDAC.csv', index=True)
